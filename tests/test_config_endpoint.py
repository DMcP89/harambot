...
async def test_config_post_handler_update_success(mock_guild, cli):
    ...
    resp = await cli.post("/api/config/123", json={
        "league_id": "67890",
        "league_type": "nba",
...
@patch("harambot.services.webserver.Guild")
async def test_config_post_handler_create_success(mock_guild, mock_yahoo_auth, cli):
    mock_guild.get_or_none.return_value = None
    mock_yahoo_auth.return_value = {"access_token": "token", "refresh_token": "refresh"}
...
@patch("harambot.services.webserver.yahoo_auth")
@patch("harambot.services.webserver.Guild")
async def test_config_post_handler_create_auth_fail(mock_guild, mock_yahoo_auth, cli):
    mock_guild.get_or_none.return_value = None
    mock_yahoo_auth.return_value = None

    resp = await cli.post("/api/config/123", json={
        "league_id": "67890",
        "yahoo_token": "invalid_token"
    })
